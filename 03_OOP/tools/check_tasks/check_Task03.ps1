...
function Check-Task {
    ...
    if ((Check-Path -Path Book.php) || (Check-Path -Path Student.php)) { $doneChecks++ } else { $failureChecks++ }
    if ((Check-Path -Path BooksList.php) || (Check-Path -Path StudentsList.php)) { $doneChecks++ } else { $failureChecks++ }
