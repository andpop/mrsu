...
От класса `Logger` нужно наследовать класс `FileLogger` и реализовать в нем метод `log()` для записи информации в текстовый файл с разделителями. Имя файла передавать в конструктор при создании экземпляра класса.
2. Добавить созданные классы в приложение для работы со списком студентов из задания 3, соблюдая принципы SOLID. Обеспечить логгирование операций создания нового списка (вызов конструктора) и добавления данных о студенте (вызов метода `add()`).
