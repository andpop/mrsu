...
1. Реализовать класс `Url`, который описывает переданный в конструктор URL-адрес (допустимые протоколы HTTP и HTTPS) и позволяет извлекать из него части (https://developer.mozilla.org/ru/docs/Learn/Understanding_URLs). Данный класс должен реализовать интерфейс `UrlInterface`, содержащий методы:
